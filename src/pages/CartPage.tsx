...
export const CartPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-6 border-b border-zinc-200 gap-4 mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-zinc-900 tracking-tight">Shopping Bag</h1>
      ...
      <div className="mb-8 p-4 sm:p-5 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 rounded-3xl border border-indigo-100 shadow-xs">
        <div className="flex items-center justify-between text-xs font-bold mb-2">
          ...
          <span className="text-indigo-600 font-mono">{Math.round(freeShippingProgress)}%</span>
        </div>
        <div className="w-full h-2.5 bg-white/80 rounded-full overflow-hidden p-0.5">
          <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          {cart.items.map((item, index) => (
            <div
              ...
              className="bg-white rounded-3xl border border-zinc-200/90 p-4 sm:p-5 shadow-xs flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between transition-all hover:border-indigo-200"
            >
              ...
              <div className="flex items-center gap-4 flex-1">
                <Link to={`/product/${item.productId}`} className="shrink-0">
                  <img
                    ...
                    className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl object-cover border border-zinc-100 bg-zinc-50"
                  />
                </Link>
                ...
                <div className="space-y-1 flex-1 min-w-0">
                  ...
                  <Link
                    ...
                    className="text-xs sm:text-sm font-extrabold text-zinc-900 line-clamp-2 hover:text-indigo-600 transition-colors block"
                  >
                    {item.name}
                  </Link>

                  {/* Attributes */}
                  <div className="flex items-center gap-2 text-[11px] text-zinc-500 font-medium pt-0.5">
                    {item.selectedSize && <span>Size: <strong className="text-zinc-800">{item.selectedSize}</strong></span>}
                    {item.selectedColor && (
                      <>
                        <span>•</span>
                  ...
                  {/* Unit Price */}
                  <div className="text-sm font-black text-zinc-900 pt-1">
              ...
              <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-3 pt-3 sm:pt-0 border-t sm:border-t-0 border-zinc-100">
                ...
                <div className="flex items-center border border-zinc-200 rounded-xl bg-zinc-50 p-1">
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center text-zinc-600 hover:text-zinc-900 disabled:opacity-30 rounded-lg hover:bg-white transition-colors"
                  >
                    -
                  </button>
                  <span className="w-8 text-center text-xs font-black font-mono text-zinc-900">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(item.productId, item.quantity + 1, item.selectedColor, item.selectedSize)}
                    className="w-7 h-7 flex items-center justify-center text-zinc-600 hover:text-zinc-900 rounded-lg hover:bg-white transition-colors"
                  >
                    +
                  </button>
                </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 bg-zinc-50 rounded-3xl border border-zinc-200/80 text-xs">
            ...
            <div className="flex items-center gap-2 text-zinc-700 col-span-2 sm:col-span-1">
        ...
        <div className="lg:col-span-5 space-y-5 sticky top-24">
          <div className="bg-white rounded-3xl border border-zinc-200/90 p-6 shadow-md space-y-6">
            <h2 className="text-lg font-black text-zinc-900 tracking-tight pb-3 border-b border-zinc-100">
              Order Summary
            </h2>
            ...
            <form onSubmit={handleApplyCoupon} className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 uppercase font-mono text-xs bg-zinc-50 border border-zinc-200 rounded-xl px-3 py-2.5 outline-none focus:border-indigo-600"
                />
                <button
                  type="submit"
                  className="px-4 py-2.5 bg-zinc-950 hover:bg-indigo-600 text-white rounded-xl text-xs font-bold transition-colors"
                >
                  Apply
                </button>
              </div>
              ...
              {appliedCoupon && (
                <div className="flex items-center justify-between text-xs bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-xl px-3 py-2">
            ...
            <div className="space-y-2.5 text-xs pt-2 border-t border-zinc-100">
              ...
              <div className="pt-3 border-t border-zinc-200 flex justify-between items-baseline">
                <span className="text-base font-black text-zinc-900">Grand Total</span>
            ...
            <Link
              to="/checkout"
              className="w-full py-4 bg-zinc-950 hover:bg-indigo-600 text-white rounded-2xl text-sm font-black flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transition-all duration-200 active:scale-95"
            >
